package com.hanium.lab8_1_2battery;
            ...
            if(action.equals(Intent.ACTION_BATTERY_CHANGED)){
                ...
                switch(status) {
                    case BatteryManager.BATTERY_STATUS_CHARGING:
                        Toast.makeText(getApplicationContext(), "배터리 상태:현재 충전중임", 0).show();
                        ;
                        break;

                    case BatteryManager.BATTERY_STATUS_NOT_CHARGING:
                        Toast.makeText(getApplicationContext(), "배터리 상태:현재 충전중 아님", 0).show();
                        ;
                        break;

                    case BatteryManager.BATTERY_STATUS_DISCHARGING:
                        Toast.makeText(getApplicationContext(), "배터리 상태:현재 방전됨", 0).show();
                        ;
                        break;

                    case BatteryManager.BATTERY_STATUS_FULL:
                        Toast.makeText(getApplicationContext(), "배터리 상태:충전 100% 완료", 0).show();
                        ;
                        break;

                    default:
                        Toast.makeText(getApplicationContext(), "배터리 상태:알수없음", 0).show();
                        ;
                        break;
                }
            }
